...
use crate::traverse::{traverse_directory, TraverseOptions};
...
fn test_omit_path_prefix_basic() -> Result<()> {
    ...
    // Verify results
    assert_eq!(results.len(), test_files.len(), "Should find all test files");
    ...
    for result in &results {
        ...
        assert!(!result.file_path.starts_with(temp_path), 
                ...
                result.file_path.display(), temp_path.display());
...
fn test_omit_path_prefix_with_pattern() -> Result<()> {
    ...
    for result in &results {
        ...
        let normalized_path = normalize_path(&result.file_path);
        assert!(normalized_path.ends_with(".rs"), 
                "File '{}' should be a Rust file", normalized_path);
...
fn test_omit_path_prefix_partial_match() -> Result<()> {
    ...
    for result in &results {
        assert!(result.file_path.starts_with(temp_path),
                ...
                result.file_path.display(), temp_path.display(), non_matching_prefix.display());
    }
...
fn test_omit_path_prefix_with_depth_limit() -> Result<()> {
    ...
    for result in &results {
        ...
        assert!(!normalized_path.contains('/'), 
                "File '{}' should be in the root directory", normalized_path);
    }
