...
",
    ...
    fn validate_pagination_consistency(
        ...
        println!(
            "Validating pagination with {} total results, page size {}, {} pages",
        ...
        for page in 0..total_pages {
            ...
            for (i, (actual, expected)) in page_results
                .lines
                .iter()
                .zip(expected_results.lines.iter())
                .enumerate()
            {
                assert_eq!(
                    actual.file_path, expected.file_path,
                    "Page {}, result {} file path mismatch",
                    page, i
                );
                assert_eq!(
                    actual.line_number, expected.line_number,
                    "Page {}, result {} line number mismatch",
                    page, i
                );
                assert_eq!(
                    actual.line_content, expected.line_content,
                    "Page {}, result {} content mismatch",
                    page, i
                );
            }

            println!(
                "✓ Page {} validated successfully ({} results)",
                page,
                page_results.lines.len()
            );
        }
    ...
    fn test_search_pagination_consistency() -> Result<()> {
        ...
        assert_eq!(
            all_results.lines.len(),
            full_results.lines.len(),
    ...
    fn test_search_sorting_behavior() -> Result<()> {
        ...
        println!(
            "Creating test files for sorting test in {}",
            temp_path.display()
        );
        create_search_files_for_pagination(temp_path)?;
        ...
        println!(
            "Found {} results, verifying sort order",
