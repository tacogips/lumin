    ...
    fn test_collect_files_nested() -> Result<()> {
        ...
        for entry in fs::read_dir(temp_path)? {
            ...
            if path.is_file() {
                ...
                println!("  {} (rel: {}) matches? {}", path.display(), rel_path.display(), matches);
            }
        }
        ...
        println!("\nNested files found by collect_files: {}", files_nested.len());
        ...
        println!("Files found with modified pattern: {}", files_nested_modified.len());
        for file in &files_nested_modified {
            println!("  {}", file.display());
        }
        
        assert_eq!(files_nested_modified.len(), 3, "Should find 3 files in nested directories with modified pattern");
