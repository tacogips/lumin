...
mod search_edge_case_tests {
    ...
    fn test_before_context_first_line_match() -> Result<()> {
        ...
        let context_lines_before = results
            .lines
            .iter()
        ...
        assert_eq!(
            ...
            "There should be no context lines before the match on the first line"
        );
    ...
    fn test_after_context_last_line_match() -> Result<()> {
        ...
        let context_lines_after = results
            .lines
            .iter()
            .filter(|r| r.is_context && r.line_number > match_result.line_number)
            .count();
        ...
        assert_eq!(
            ...
            "There should be no context lines after the match on the last line"
        );
    ...
    fn test_combined_context_first_and_last_line_matches() -> Result<()> {
        ...
        let first_match = results
            .lines
            .iter()
            .find(|r| !r.is_context && r.line_number == 1)
            .unwrap();
        let last_match = results
            .lines
            .iter()
            .find(|r| !r.is_context && r.line_number == 5)
            .unwrap();

        // Verify first match
        let context_lines_before_first = results
            .lines
            .iter()
        ...
        // Verify last match
        let context_lines_after_last = results
            .lines
            .iter()
        ...
        assert_eq!(
            context_lines_before_first, 0,
            "There should be no context lines before the match on the first line"
        );
        assert_eq!(
            context_lines_after_last, 0,
            "There should be no context lines after the match on the last line"
        );

        // Check that context between matches is present
        let lines_between = results
            .lines
            .iter()
            .filter(|r| {
                r.is_context
                    && r.line_number > first_match.line_number
                    && r.line_number < last_match.line_number
            })
            .count();

        assert_eq!(
            lines_between, 3,
            "All lines between first and last match should be included as context"
        );
    ...
    fn test_single_line_file_match() -> Result<()> {
        ...
        assert_eq!(
            results.lines.iter().filter(|r| r.is_context).count(),
            0,
