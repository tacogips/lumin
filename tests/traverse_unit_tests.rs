...
fn test_traverse_without_gitignore_respect() -> Result<()> {
    ...
    assert!(file_paths.iter().any(|path| path.contains(".hidden/secret.txt")));
...
fn test_traverse_with_glob_pattern() -> Result<()> {
    ...
    assert!(!results.is_empty());
    assert!(results.iter().all(|r| r.file_path.to_string_lossy().ends_with(".txt")));
    
    // Should not find other text files like .md or .rs
    assert!(results.iter().all(|r| !r.file_path.to_string_lossy().ends_with(".md")));
    assert!(results.iter().all(|r| !r.file_path.to_string_lossy().ends_with(".rs")));
...
fn test_traverse_with_substring_pattern() -> Result<()> {
    ...
    assert!(!results.is_empty());
    assert!(results.iter().all(|r| r.file_path.to_string_lossy().contains("level")));
...
fn test_is_hidden_check() -> Result<()> {
    ...
    for file in hidden_files {
        assert!(file.is_hidden(), "File in .hidden directory should be marked as hidden");
    }
