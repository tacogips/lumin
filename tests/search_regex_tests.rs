...
fn test_search_with_word_boundaries() -> Result<()> {
    ...
    let exact_matches = results.iter()
...
fn test_search_email_pattern() -> Result<()> {
    ...
    assert!(results.iter().any(|r| r.line_content.contains("user@example.com")));
    assert!(results.iter().any(|r| r.line_content.contains("another.user@domain.co.uk")));
...
fn test_search_with_escaping() -> Result<()> {
    ...
    assert!(results.iter().any(|r| r.line_content.contains("Special characters: .*")));
...
fn test_search_with_non_greedy_matching() -> Result<()> {
    ...
    assert!(results.iter().any(|r| r.line_content.contains("Basic text: apple orange banana")));
