...
fn test_content_omission() -> Result<()> {
    ...
    let content =
        "0123456789abcdefghijklmnopqrstuvwxyz_PATTERN_0123456789abcdefghijklmnopqrstuvwxyz";
    ...
    println!(
        "content_omitted flag: {}",
        omitted_results.lines[0].content_omitted
    );
    ...
    assert!(
        omitted_content.contains("_PATTERN_"),
        "Omitted content should contain the matched pattern"
    );
    assert!(
        omitted_content.contains("vwxyz_PATTERN_"),
        "Omitted content should contain context before pattern"
    );
    assert!(
        omitted_content.ends_with("<omit>"),
        "Omitted content should end with <omit> marker"
    );
    ...
    assert!(
        omitted_content2.contains("_PATTERN_"),
        "Omitted content should contain the pattern"
    );

    // With 20 chars of context, we should see more before and after the pattern
    assert!(
        omitted_content2.contains("klmnopqrstuvwxyz_PATTERN_"),
        "Should have more context before pattern"
    );
    assert!(
        omitted_content2.contains("_PATTERN_0123456789abcdef"),
        "Should have more context after pattern"
    );
    assert!(
        omitted_content2.ends_with("<omit>"),
        "Omitted content should end with <omit> marker"
    );
    ...
    let long_match_results = search_files(
        "verylongpatternstring",
        temp_dir.path(),
        &small_omit_options,
    )?;

    // Find the result for the long_match.txt file
    let long_match_result = long_match_results
        .lines
        .iter()
    ...
    println!(
        ...
        long_match_result.line_content.trim()
    );
    ...
    assert!(
        trimmed.contains("VERYLONGPATTERNSTRING") || trimmed.contains("verylongpatternstring"),
        "Should contain the complete match string"
    );

    // But should omit context far from the match
    assert!(
        trimmed.contains("<omit>"),
        "Should have omitted some content"
    );
    assert!(
        !trimmed.contains("This contains") || !trimmed.contains("should be truncated"),
    ...
    let multi_match_result = multi_results
        .lines
        .iter()
