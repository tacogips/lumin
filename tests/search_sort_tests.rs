    ...
    fn test_search_results_sorted() -> Result<()> {
        ...
        for (i, line) in result.lines.iter().enumerate() {
            ...
            if let Some(prev) = prev_path {
                assert!(
                    prev <= &line.file_path,
                    "Results not sorted by file path at index {}",
                    i
                );
                ...
                if prev == &line.file_path {
                    ...
                    assert!(
                        prev_num < line.line_number,
                        "Results not sorted by line number at index {}",
                        i
                    );
                }
            }
        ...
        assert!(
            result.lines[0]
                ...
                .to_string_lossy()
                .contains("a_file.txt"),
            "First result should be from a_file.txt, got: {}",
            result.lines[0].file_path.display()
        );

        // Check the expected order of line numbers for the first file
        assert_eq!(
            result.lines[0].line_number, 2,
            "First match in a_file.txt should be on line 2"
        );
        assert_eq!(
            result.lines[1].line_number, 4,
            "Second match in a_file.txt should be on line 4"
        );
        assert_eq!(
            result.lines[2].line_number, 6,
            "Third match in a_file.txt should be on line 6"
        );
        ...
        assert!(
            unsorted_result.lines[0]
                .file_path
                .to_string_lossy()
                .contains("a_file.txt")
        );
        assert!(
            unsorted_result.lines[2]
                .file_path
                .to_string_lossy()
                .contains("m_file.txt")
        );
        assert!(
            unsorted_result.lines[4]
                .file_path
                .to_string_lossy()
                .contains("z_file.txt")
        );
