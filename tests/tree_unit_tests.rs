use anyhow::Result;
use lumin::tree::{TreeOptions, generate_tree, Entry};
use std::path::Path;
...
fn test_tree_basic() -> Result<()> {
    ...
    if let Some(root) = root_dir {
        ...
        let dir_names: Vec<_> = root.entries.iter().filter_map(|e| {
...
fn test_tree_without_gitignore_respect() -> Result<()> {
    ...
    assert!(contains_hidden_dir, ".hidden directory should be included when not respecting gitignore");
    ...
    if let Some(dir) = hidden_dir {
        ...
        assert!(has_secret_file, "hidden/secret.txt should be included when not respecting gitignore");
    }
...
fn test_tree_structure_integrity() -> Result<()> {
    ...
    let has_level1 = results.iter().any(|d| d.dir.contains("level1") && !d.dir.contains("level2"));
    ...
    let level1_dir = results.iter().find(|d| d.dir.contains("level1") && !d.dir.contains("level2"));
    if let Some(level1) = level1_dir {
        ...
        assert!(has_level2_entry, "level1 directory should contain level2 as a subdirectory");
    }
    ...
    if let Some(dir) = text_files_dir {
        ...
        let file_names: Vec<_> = dir.entries.iter().filter_map(|e| {
            if let Entry::File { name } = e {
                Some(name.as_str())
            } else {
                None
        ...
        assert!(file_names.contains(&"sample.txt"), "text_files directory should contain sample.txt");
        assert!(file_names.contains(&"markdown.md"), "text_files directory should contain markdown.md");
        assert!(file_names.contains(&"config.toml"), "text_files directory should contain config.toml");
    }
